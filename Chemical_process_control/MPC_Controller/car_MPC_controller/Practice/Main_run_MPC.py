...
import support_files_car as sup
import matplotlib.gridspec as gridspec
import matplotlib.animation as animation

# Create an object for the support functions
from MAIN_MPC_car_lateral import PID_switch

support = sup.SupportFilesCar()
const = support.constants

# Load the constant values needed in the main file
Ts = const[7]
outputs = const[11] # number of outputs (psi, Y)
hz = const[12] # horizon prediction period
x_dot = const[13] # constant longitudinal velocity
time_length = const[16] # duration of the manoeuvre
...
psi_opt_total = np.zeros((len(t), hz)) # optimum point
Y_opt_total = np.zeros((len(t), hz)) # optimum point
...
for i in range(0, sim_length-1):
    ...
    # Generate the augment current state and the reference vector
    x_aug_t = np.transpose([np.concatenate((states, [U1]), axis=0)])
    #Ts=0.1
    # From the refSignals vector, only extract the reference values fron
    # your [current sample (NOW) +Ts] to [Now + horizon period (hz)]
    ...
    # With each loop , it all shifts by 0.1 seconds because Ts = 0.1 s
    k = k + outputs
    ...
    # Generate the compact simplification
...
###################################### PID #####################################################################
    ...
    if PID_switch == 1:
        ...
        if i>0:
            ...
            e_dot_pid_taw = (e_pid_yaw_i - e_pid_yaw_im1)/Ts
            e_init_pid_yaw = e_init_pid_yaw + (e_pid_yaw_im1 + e_pid_yaw_i)/2*Ts
            Kp_yaw = const[20]
            Kd_yaw = const[21]
            Ki_yaw = const[22]
            U1_yaw = Kp_yaw*e_pid_yaw_im1+Kd_yaw*e_dot_pid_taw+ Ki_yaw*e_int_pid_yaw

            e_pid_yaw_im1 = Y_ref[i-1]-old_states[3]
            e_pid_Y_i = Y_ref[i] - states[3]
            e_dot_pid_Y = (e_pid_Y_i-e_pid_yaw_im1)/Ts
            e_int_pid_Y = e_int_pid_Y + (e_pid_yaw_im1)
            Kp_Y = const[23]
            Kd_Y = const[24]
            Ki_yaw = const[25]
...
lf=constants[5]
lr=constants[6]
cars_distance=constants[18]
# print(frame_amount)
def update_plot(num):
    hz = constants[12] # horizon prediction period
